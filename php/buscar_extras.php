...
<table class="bordered highlight responsive-table">
    ...
    <tbody>
      ...
      while($user = mysqli_fetch_array($sql_tmp)){
        ...
        $sql_extras = mysqli_query($conn, "SELECT * FROM horas_extras WHERE fecha >= '$ValorDe' AND fecha <= '$ValorA' AND (usuario = $id OR apoyo = $id OR apoyomas LIKE '%$user_name%')");
        if (mysqli_num_rows($sql_extras)) {
          ...
          $total = mysqli_fetch_array(mysqli_query($conn, "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(tiempo))) suma FROM horas_extras WHERE fecha >= '$ValorDe' AND fecha <= '$ValorA' AND (usuario = $id OR apoyo = $id OR apoyomas LIKE '%$user_name%')"));
