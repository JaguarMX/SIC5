    ...
    if (mysqli_num_rows($Tmp)>0) {
        ...
        while ($CLIENTE_S = mysqli_fetch_array($Tmp)) {
            ...
            for ($x = 1; $x < 11; $x++) {
                ...
                if (count($ARRAY)>0) {
                    #SI EN CUENTRA LA IP MODIFICAMOS EL ESTAUS DE CLIENTE cortado = 1 y en cuantas veces = $x
                    mysqli_query($conn, "UPDATE tmp_cortes SET cortado = 1, veces = '$x' WHERE id = '$Id' AND ip = '$IP_S'");
                    #SI LO ENCUENTRA ROPEMOS EL CICLO CON BREAK ANTES DE HACER LAS 10 ITERACIONES
                    break;
                }
            }
        }
...
$Tmp_list = mysqli_query($conn, "SELECT * FROM tmp_cortes WHERE cortado = 1");
...
$Tmp_list_no = mysqli_query($conn, "SELECT * FROM tmp_cortes WHERE cortado = 0");
#CONTAMOS CUANTOS CLIENTES SON
...
<div><br><br><br><hr>
    ...
    <button class="btn waves-light waves-effect right pink" onclick="iniciarCorte(<?php echo $Servidor; ?>);"><i class="material-icons prefix right">signal_wifi_off</i>Iniciar</button>
   <div class="row" id="iniciarCorte">   </div> <br>
</div>