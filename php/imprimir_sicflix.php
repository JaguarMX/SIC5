...
class PDF extends FPDF{
    ...
    $reporte = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `reporte_sicflix` WHERE cliente = $id_cliente AND descripcion = 'Activar Sicflix' AND estatus != 0 ORDER BY id DESC LIMIT 1"));
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    $pdf->SetY($pdf->GetY()+2);
    $pdf->SetX(5);
    $pdf->SetFont('Helvetica','', 8);
    ...
    //CUADRO DEL REPORTE
    $CR7 = '';
    if ($reporte['fecha_atendio'] == $pago['fecha']) {
        $CR7 = 'USUARIO SICFLIX:'."\n".$reporte['nombre_usuario_sicflix']."\n".'CONTRASEÑA SICFLIX:'."\n".$reporte['contraseña_sicflix'];
    }else{
        $CR7 = 'USUARIO SICFLIX:'."\n".'PREVIAMENTE ASIGNADO'."\n".'CONTRASEÑA SICFLIX:'."\n".'PREVIAMENTE ASIGNADA';
    }
    $pdf->MultiCell(60,5,utf8_decode($CR7),1,'L',0);
    ...
    }elseif ($pago['tipo'] == 'SICFLIX'){
        $msj = 'RECOMENDACIONES:'."\n"."\n".'1. DEBE REALIZAR SU PROXIMO PAGO ANTES         DEL:  '.date_format(new \DateTime($cliente['fecha_corte_sicflix']), "d/m/Y")."\n".'2. CONTAR CON LINEA REGULADA                              (REGULADOR DE CORRIENRE).'."\n".'3. NO MODIFICAR EL ORDEN DEL CABLEADO.'."\n".'4. NO PRESIONAR BOTONES DEL MODEM.'."\n".'5. EN CASO DE ALGUNA "FALLA"                                COMUNICARSE AL 433 935 6286.';
    }
    $pdf->SetY($pdf->GetY()+3);
    $pdf->SetX(5);
    $pdf->SetFont('Helvetica','B', 8);
