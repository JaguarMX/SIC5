...
if($area['area']!="Administrador"){
  ...
  $Pago = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM pagos_sicflix WHERE id = $IdPago"));
  ...
  $INFO_Pgo =  mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM pagos_sicflix WHERE id = $IdPago"));
  ...
  if(mysqli_query($conn, "INSERT INTO pagos_borrados(cliente, cantidad, descripcion, realizo, tipo_cambio, fecha_hora_registro, motivo, borro, fecha_borrado) VALUES ($Cliente,'$Cantidad', '$Descripcion', $Realizo, '$Tipo_Cambio', '$Fecha_registro', '$Motivo', $id, '$HOY')")){
    ...
    if(mysqli_query($conn, "DELETE FROM pagos_sicflix WHERE id = '$IdPago'")){
      echo '<script >M.toast({html:"Pago Borrado.", classes: "rounded"})</script>'; 
      if ($Tipo == 'Mensualidad') {
        $ultimoPago =  mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM pagos_sicflix WHERE id_cliente = $IdCliente AND tipo = 'Mensualidad' ORDER BY id DESC LIMIT 1"));
...
$sql_pagos = "SELECT * FROM pagos_sicflix  WHERE id_cliente = '$IdCliente' AND tipo = '$Tipo' ORDER BY id DESC";
